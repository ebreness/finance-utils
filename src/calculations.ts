...
import type { AmountCents, BasisPoints, TaxCalculationResult } from './types.ts';
 ...
 */
export function calculateTaxFromBase(baseCents: AmountCents, taxBasisPoints: BasisPoints): AmountCents {
 ...
 */
export function calculateBaseFromTotal(totalCents: AmountCents, taxBasisPoints: BasisPoints): AmountCents {
...
export function calculateTaxBreakdown(
  ...
  const taxAmountCents = totalCents - baseAmountCents;
  
  // Verify exact precision (this should always be true due to calculateBaseFromTotal's design)
  if (baseAmountCents + taxAmountCents !== totalCents) {
    throw new Error(`Tax breakdown precision error: base ${baseAmountCents} + tax ${taxAmountCents} = ${baseAmountCents + taxAmountCents} ≠ total ${totalCents}`);
  }
